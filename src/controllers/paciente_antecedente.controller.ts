...
class Paciente_AntecedenteController {
  ...
  public async getPaciente_AntecedenteById(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const paciente_antecedente = await Paciente_AntecedenteService.getPaciente_AntecedenteById(String(id));
  ...
  public async getPaciente_AntecedenteByPacienteId(req: Request, res: Response) {
    try {
      ...
      const paciente_antecedente = await Paciente_AntecedenteService.getPaciente_AntecedenteByPacienteId(String(pacienteId));
      res.status(200).json(paciente_antecedente);
    } catch (error) {
      res.status(404).json((error as Error).message);
    }
  ...
  public async updatePaciente_Antecedente(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const data = req.body;
      const updatedPaciente_Antecedente = await Paciente_AntecedenteService.updatePaciente_Antecedente(String(id), data);
  ...
  public async deletePaciente_Antecedente(req: Request, res: Response) {
    try {
      const { id } = req.params;
      await Paciente_AntecedenteService.deletePaciente_Antecedente(String(id));
